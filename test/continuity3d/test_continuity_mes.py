...
def setup1(Lx, depth, mimetic=True):
    """
...
def setup1dg(Lx, depth):
    """Linear bath, zero elev, constant uv"""
    return setup1(Lx, depth, mimetic=False)


def setup2(Lx, depth, mimetic=True):
    """
    ...
    v_str = '0.2*sin(0.2*pi*(1.0*x[0] + 3.0*x[1])/Lx)'
    w_str = 'h0*(-0.12*pi*cos(0.2*pi*(1.0*x[0] + 3.0*x[1])/Lx)/Lx - 0.6*pi*cos(0.2*pi*(3.0*x[0] + 1.0*x[1])/Lx)/Lx) + x[2]*(-0.12*pi*cos(0.2*pi*(1.0*x[0] + 3.0*x[1])/Lx)/Lx - 0.6*pi*cos(0.2*pi*(3.0*x[0] + 1.0*x[1])/Lx)/Lx)'
    out['uv_expr'] = Expression(
        (
            u_str,
            v_str,
...
def setup3dg(Lx, depth):
    """Non-trivial bath and elev, u=1, v=0"""
    return setup2(Lx, depth, mimetic=False)


def setup4(Lx, depth, mimetic=True):
...
def setup4dg(Lx, depth):
    """Non-trivial bath and elev, uv depends on (x,y)"""
    return setup3(Lx, depth, mimetic=False)


def setup5(Lx, depth, mimetic=True):
...
def setup5dg(Lx, depth):
    """Non-trivial bath and elev, uv depends on (x,y,z)"""
    return setup4(Lx, depth, mimetic=False)

#def setup4(Lx, depth):
    #"""Constant bath and elev, uv depends on (x,y,z)"""
    ...
    #uv_expr = Expression(
        #(
            #'sin(pi*(3*x[0] + x[1])/(5*Lx))*cos(pi*x[2]/h0)',
            #'sin(pi*(x[0] + 3*x[1])/(5*Lx))*cos(pi*x[2]/h0)/5',
            #'0.0',
        #), Lx=Lx, h0=depth)
    #w_expr = Expression(
        #(
            #'0.0',
            #'0.0',
            #'-3*h0*sin(pi*x[2]/h0)*cos(pi*(x[0] + 3*x[1])/(5*Lx))/(25*Lx) - 3*h0*sin(pi*x[2]/h0)*cos(pi*(3*x[0] + x[1])/(5*Lx))/(5*Lx)',
        #), Lx=Lx, h0=depth)
    #return bath_expr, elev_expr, uv_expr, w_expr

#def setup5(Lx, depth):
    #"""Linear bath, zero elev, uv depends on (x,y,z)"""
    #bath_expr = Expression(
        #'h0*(1 - x[0]/(2*Lx) + x[1]/(5*Lx))',
        #Lx=Lx, h0=depth)
    #elev_expr = Expression(
        #'0',
        #Lx=Lx, h0=depth)
    ...
    #w_expr = Expression(
        #(
            #'0.0',
            #'0.0',
            #'-3*h0*sin(pi*(1 - x[0]/(2*Lx) + x[1]/(5*Lx)))*cos(pi*(x[0] + 3*x[1])/(5*Lx))/(25*Lx) - 3*h0*sin(pi*(1 - x[0]/(2*Lx) + x[1]/(5*Lx)))*cos(pi*(3*x[0] + x[1])/(5*Lx))/(5*Lx) - h0*sin(pi*(x[0] + 3*x[1])/(5*Lx))*cos(pi*(1 - x[0]/(2*Lx) + x[1]/(5*Lx)))/(25*Lx) + h0*sin(pi*(3*x[0] + x[1])/(5*Lx))*cos(pi*(1 - x[0]/(2*Lx) + x[1]/(5*Lx)))/(2*Lx) - 3*h0*sin(pi*x[2]/h0)*cos(pi*(x[0] + 3*x[1])/(5*Lx))/(25*Lx) - 3*h0*sin(pi*x[2]/h0)*cos(pi*(3*x[0] + x[1])/(5*Lx))/(5*Lx)',
        #), Lx=Lx, h0=depth)
    #return bath_expr, elev_expr, uv_expr, w_expr

#def setup6(Lx, depth):
    #"""Complex bath, zero elevation, uv depends on (x,y,z)
    #NOTE FIXME this fails"""
    #bath_expr = Expression(
    #'h0*(cos(pi*sqrt(x[0]*x[0] + x[1]*x[1] + 1)/Lx) + 3)/4',
    #Lx=Lx, h0=depth)
    #elev_expr = Expression(
        #'0',
        #Lx=Lx, h0=depth)
    ...
    #w_expr = Expression(
        #(
            #'0.0',
            #'0.0',
            #'pi*h0*x[0]*sin(pi*(3*x[0] + x[1])/(5*Lx))*sin(pi*sqrt(x[0]*x[0] + x[1]*x[1] + 1)/Lx)*cos(pi*(cos(pi*sqrt(x[0]*x[0] + x[1]*x[1] + 1)/Lx)/4 + 3/4))/(4*Lx*sqrt(x[0]*x[0] + x[1]*x[1] + 1)) + pi*h0*x[1]*sin(pi*(x[0] + 3*x[1])/(5*Lx))*sin(pi*sqrt(x[0]*x[0] + x[1]*x[1] + 1)/Lx)*cos(pi*(cos(pi*sqrt(x[0]*x[0] + x[1]*x[1] + 1)/Lx)/4 + 3/4))/(20*Lx*sqrt(x[0]*x[0] + x[1]*x[1] + 1)) - 3*h0*sin(pi*(cos(pi*sqrt(x[0]*x[0] + x[1]*x[1] + 1)/Lx)/4 + 3/4))*cos(pi*(x[0] + 3*x[1])/(5*Lx))/(25*Lx) - 3*h0*sin(pi*(cos(pi*sqrt(x[0]*x[0] + x[1]*x[1] + 1)/Lx)/4 + 3/4))*cos(pi*(3*x[0] + x[1])/(5*Lx))/(5*Lx) - 3*h0*sin(pi*x[2]/h0)*cos(pi*(x[0] + 3*x[1])/(5*Lx))/(25*Lx) - 3*h0*sin(pi*x[2]/h0)*cos(pi*(3*x[0] + x[1])/(5*Lx))/(5*Lx)',
        #), Lx=Lx, h0=depth)
    #return bath_expr, elev_expr, uv_expr, w_expr

#def setup7(Lx, depth):
    #"""Non-lin bath, zero elev, vertically varying uv
    #NOTE FIXME this fails"""
    ## NOTE in this test the div term vanishes -- w is just the bottom BC
    #bath_expr = Expression(
        #'h0*(sin(pi*x[0]/Lx) + 3.0)/4.0',
        #Lx=Lx, h0=depth)
    #elev_expr = Expression(
        #'0',
        #Lx=Lx, h0=depth)
    #uv_expr = Expression(
        #(
            ...
            #'0.0',
        #), Lx=Lx, h0=depth)
    ## FIXME this fails because Expression is not evaluated correctly
    #w_expr = Expression(
        #(
            #'0.0',
            #'0.0',
            #'-pi*h0*cos(pi*(sin(pi*x[0]/Lx)/4.0 + 3.0/4.0))*cos(pi*x[0]/Lx)/(4.0*Lx)',
        #), Lx=Lx, h0=depth)
    #return bath_expr, elev_expr, uv_expr, w_expr
...
#def setup8(Lx, depth):
    ...
    #bath_expr = Expression(
        ...
        #Lx=Lx, h0=depth)
    #elev_expr = Expression(
        #'5*sin(pi*sqrt(x[0]*x[0] + x[1]*x[1] + 1)/(4*Lx))',
        #Lx=Lx, h0=depth)
    #uv_expr = Expression(
        #(
            #'(1 - x[2]/h0)*sin(pi*(3*x[0] + x[1])/(5*Lx))',
            #'(1 - x[2]/h0)*sin(pi*(x[0] + 3*x[1])/(5*Lx))/5',
            #'0.0',
        #), Lx=Lx, h0=depth)
    #w_expr = Expression(
        #(
            #'0.0',
            #'0.0',
            #'pi*h0*x[0]*(cos(pi*sqrt(x[0]*x[0] + x[1]*x[1] + 1)/Lx)/4 + 7/4)*sin(pi*(3*x[0] + x[1])/(5*Lx))*sin(pi*sqrt(x[0]*x[0] + x[1]*x[1] + 1)/Lx)/(4*Lx*sqrt(x[0]*x[0] + x[1]*x[1] + 1)) + pi*h0*x[1]*(cos(pi*sqrt(x[0]*x[0] + x[1]*x[1] + 1)/Lx)/4 + 7/4)*sin(pi*(x[0] + 3*x[1])/(5*Lx))*sin(pi*sqrt(x[0]*x[0] + x[1]*x[1] + 1)/Lx)/(20*Lx*sqrt(x[0]*x[0] + x[1]*x[1] + 1)) - h0*(3*pi*cos(pi*x[0]/(5*Lx) + 3*pi*x[1]/(5*Lx)) + 15*pi*cos(3*pi*x[0]/(5*Lx) + pi*x[1]/(5*Lx)))*pow(cos(pi*sqrt(x[0]*x[0] + x[1]*x[1] + 1)/Lx) + 3, 2)/(800*Lx) - h0*(3*pi*cos(pi*x[0]/(5*Lx) + 3*pi*x[1]/(5*Lx)) + 15*pi*cos(3*pi*x[0]/(5*Lx) + pi*x[1]/(5*Lx)))*(cos(pi*sqrt(x[0]*x[0] + x[1]*x[1] + 1)/Lx) + 3)/(100*Lx) - x[2]*(3*pi*cos(pi*x[0]/(5*Lx) + 3*pi*x[1]/(5*Lx)) + 15*pi*cos(3*pi*x[0]/(5*Lx) + pi*x[1]/(5*Lx)))/(25*Lx) + x[2]*x[2]*(3*pi*cos(pi*x[0]/(5*Lx) + 3*pi*x[1]/(5*Lx)) + 15*pi*cos(3*pi*x[0]/(5*Lx) + pi*x[1]/(5*Lx)))/(50*Lx*h0)',
        #), Lx=Lx, h0=depth)
    #return bath_expr, elev_expr, uv_expr, w_expr
...
def run_scaling(setup, ref_list, order, expected_slope=None, export=False):
    """Runs test for a list of refinements and computes error convergence rate"""
    l2_err = []
    for r in ref_list:
        l2_err.append(run(setup, r, order, export=export))
    x = numpy.log(numpy.array(ref_list))
    y = -numpy.log(numpy.array(l2_err))
    slope, intercept, r_value, p_value, std_err = stats.linregress(x, y)

    slope_rtol = 0.2
    setup_name = setup.__name__
    print '{:}: convergence rate {:.4f}'.format(setup_name, slope)
    if expected_slope is not None:
        ...
        print 'PASSED'.format(setup_name, slope)
    return slope
...
def run_scaling(setup, ref_list, order, export=False, savePlot=False):
    ...
    check_convergence(x_log, y_log, order, 'w', savePlot)
