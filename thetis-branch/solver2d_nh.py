    ...
    **Example**
    ...
    def create_functions(self):
        ...
        for k in range(self.options.n_layers):
            ...
            setattr(self, 'uvw_' + str(k+1), Function(self.function_spaces.uvw_2d))
            ...
            if k == 0:
                ...
                uvw_fs_list.append(self.function_spaces.uvw_2d)
    ...
    def create_equations(self):
        ...
        self.eq_uv_mom = momentum2d_nh.ShallowWaterMomentumEquation_hori(
            TestFunction(self.function_spaces.U_2d),
            self.function_spaces.U_2d,
            self.function_spaces.H_2d,
            self.bathymetry_dg,
            self.options)
        self.eq_w_mom = momentum2d_nh.ShallowWaterMomentumEquation_vert(
        ...
        self.eq_free_surface = shallowwater_nh.FreeSurfaceEquation(
            ...
            self.bathymetry_dg,
            self.options)
        self.eq_la_mom = momentum2d_nh.ShallowWaterMomentumEquation_hori(
            TestFunction(self.function_spaces.uvw_2d),
            self.function_spaces.uvw_2d,
            self.function_spaces.H_2d,
            self.bathymetry_dg,
            self.options)
    ...
    def iterate(self, update_forcings=None,
        ...
        while self.simulation_time <= self.options.simulation_end_time + t_epsilon:
            ...
            hydrostatic_solver_2d = not self.options.use_nh_solver
            # --- Hydrostatic solver ---
            if hydrostatic_solver_2d:
                ...
                if consider_vertical_mass_flux:
                    for k in range(self.options.n_layers + 1):
                        ...
                        m_dic['z_'+str(k)] = w_dic['z_'+str(k)] - (z_dic['z_'+str(k)] - z_old_dic['z_'+str(k)])/self.dt - \
                                             inner(u_dic['z_'+str(k)], grad(z_old_dic['z_'+str(k)]))

                if self.simulation_time <= t_epsilon:
                    if self.options.n_layers >= 2:
                        timestepper_dic = {}
                        for k in range(self.options.n_layers - 1):
                            timestepper_dic['uv_layer_'+str(k+1)] = timeintegrator.CrankNicolson(self.eq_uv_mom, getattr(self, 'uv_av_'+str(k+1)),
                                                              self.field_dic, self.dt,
                                                              ...
                                                              theta=self.options.timestepper_options.implicitness_theta)
                            if consider_vertical_mass_flux:
                                uv_mass_term = (m_dic['z_'+str(k+1)]*(u_dic['z_'+str(k+1)] - getattr(self, 'uv_av_'+str(k+1))) -
                                                m_dic['z_'+str(k)]*(u_dic['z_'+str(k)] - getattr(self, 'uv_av_'+str(k+1))))/h_layer[k+1]
                                timestepper_dic['uv_layer_'+str(k+1)].F += self.dt*inner(uv_mass_term, uv_test)*dx
                                timestepper_dic['uv_layer_'+str(k+1)].update_solver()
                        #for k in range(self.options.n_layers):
                         #   timestepper_dic['w_layer_'+str(k+1)] = timeintegrator.CrankNicolson(self.eq_w_mom, getattr(self, 'w_'+str(k)+str(k+1)),
                          #                                    self.field_dic, self.dt,
                           #                                   bnd_conditions=self.bnd_functions['momentum'],
                            #                                  solver_parameters=self.options.timestepper_options.solver_parameters_momentum,
                             #                                 semi_implicit=self.options.timestepper_options.use_semi_implicit_linearization,
                              #                                theta=self.options.timestepper_options.implicitness_theta)
                            #if consider_vertical_mass_flux:
                             #   w_mass_term = 0.5*(m_dic['z_'+str(k+1)] + m_dic['z_'+str(k)])*(w_dic['z_'+str(k+1)] - w_dic['z_'+str(k)])/h_layer[k+1]
                ...
                # build the solver for the mixed Poisson equation
                if self.simulation_time <= t_epsilon:
                    ...
                    for k in range(self.options.n_layers):
                        ...
                        for i in range(k+1):
                            vert_vel_term += 4.*(-1)**i*(getattr(self, 'w_'+str(k-i)+str(k+1-i)) + self.dt*(q[k-i] - q[k+1-i])/h_layer[k+1-i])*q_test[k]*dx
                        ...
                        if consider_vert_adv: # TODO if make sure that considering vertical advection is benefitial, delete this logical variable
                            ...
                            if consider_vertical_mass_flux:
                                vert_vel_term += -2.*self.dt/h_layer[k+1]*inner(m_dic['z_'+str(k+1)]*(w_dic['z_'+str(k+1)] - getattr(self, 'w_'+str(k)+str(k+1))) -
                                                                                m_dic['z_'+str(k)]*(w_dic['z_'+str(k)] - getattr(self, 'w_'+str(k)+str(k+1))), q_test[k])*dx
                        # weak form of RHS terms
                        grad_1_bot = -2.*(-1)**(k+1)*(2.-h_layer[2]/(h_layer[1] + h_layer[2]))*grad(z_dic['z_'+str(0)])
                        grad_2_bot = -2.*(-1)**(k+1)*(-h_layer[1]/(h_layer[1] + h_layer[2]))*grad(z_dic['z_'+str(0)])
                        if self.options.n_layers == 1:
                            grad_1_bot = -2.*(-1)**(k+1)*grad(-self.bathymetry_dg)
                            w_bot_term = dot(grad_1_bot, getattr(self, 'uv_av_'+str(1)))*q_test[k]*dx - \
                                                 0.5*self.dt*(-div(grad_1_bot*q_test[k])*(q[0]+q[1]))*dx - \
                                                 self.dt*(1./h_layer[1]*dot(grad_1_bot, grad(z_dic['z_'+str(0)+str(1)]))*(q[0]-q[1]))*q_test[k]*dx
                        else:
                            w_bot_term = (dot(grad_1_bot, getattr(self, 'uv_av_'+str(1))) + dot(grad_2_bot, getattr(self, 'uv_av_'+str(2))))*q_test[k]*dx - \
                                                 0.5*self.dt*(-div(grad_1_bot*q_test[k])*(q[0]+q[1]) - div(grad_2_bot*q_test[k])*(q[1]+q[2]))*dx - \
                                                 self.dt*(1./h_layer[1]*dot(grad_1_bot, grad(z_dic['z_'+str(0)+str(1)]))*(q[0]-q[1]) + 
                                                          1./h_layer[2]*dot(grad_2_bot, grad(z_dic['z_'+str(1)+str(2)]))*(q[1]-q[2]))*q_test[k]*dx
                        if k == 0: # i.e. the layer adjacent to the bottom
                            if self.options.n_layers == 1:
                                ...
                                grad_1_layer1 = -(2.-h_layer[k+2]/(h_layer[k+1] + h_layer[k+2]))*grad(z_dic['z_'+str(k)]) + \
                                                h_layer[k+2]/(h_layer[k+1] + h_layer[k+2])*grad(z_dic['z_'+str(k)])
                                #grad_1_layer1 = grad(2.*z_dic['z_'+str(k)] + h_layer[k+1]*h_layer[k+2]/(h_layer[k+1] + h_layer[k+2]))
                                grad_2_layer1 = grad(h_layer[k+1]*(z_dic['z_'+str(k)]+z_dic['z_'+str(k+1)])/(h_layer[k+1] + h_layer[k+2]))
                                ...
                                interface_term += w_bot_term
                        elif k == self.options.n_layers - 1: # i.e. the layer adjacent to the free surface
                            grad_1_layern = -h_layer[k]/(h_layer[k] + h_layer[k+1])*grad(z_dic['z_'+str(k)]) + \
                                            (2.-h_layer[k]/(h_layer[k] + h_layer[k+1]))*grad(z_dic['z_'+str(k+1)])
                ...
                for k in range(self.options.n_layers):
                    if self.options.n_layers > 1:
                        getattr(self, 'q_'+str(k)).assign(self.q_mixed.split()[k])
                    if k == self.options.n_layers - 1:
                ...
                if self.simulation_time <= t_epsilon:
                    ...
                    for k in range(self.options.n_layers):
                        ...
                        if k == self.options.n_layers - 1:
                            l += inner(uv_2d - self.dt/h_mid*(getattr(self, 'q_'+str(0))*grad(z_dic['z_'+str(0)]) - 
                                                              getattr(self, 'q_'+str(k+1))*grad(z_dic['z_'+str(k+1)])), uv_test)*dx
                    prob = LinearVariationalProblem(a, l, uv_2d)
                    solver_uv_2d = LinearVariationalSolver(prob)
                solver_uv_2d.solve()
                self.uv_2d_mid.assign(uv_2d)
                # update layer-averaged horizontal velocity self.uv_av_{k+1}
                if self.simulation_time <= t_epsilon and self.options.n_layers >= 2:
                    solver_uv_la = []
                    a = inner(uv_tri, uv_test)*dx
                    for k in range(self.options.n_layers - 1):
                        ...
                        prob = LinearVariationalProblem(a, l, getattr(self, 'uv_av_'+str(k+1)))
                        solver_uv_la.append(LinearVariationalSolver(prob))
                if self.options.n_layers >= 2:
                    sum_uv_av = 0.
                    for k in range(self.options.n_layers - 1):
                        solver_uv_la[k].solve()
                        sum_uv_av = sum_uv_av + alpha[k]*getattr(self, 'uv_av_'+str(k+1))
                    # update layer-averaged velocity of the free-surface layer
                    getattr(self, 'uv_av_'+str(self.options.n_layers)).project((uv_2d - sum_uv_av)/alpha[self.options.n_layers-1])
                # update layer-averaged vertical velocity self.w_{k}{k+1}
                if self.simulation_time <= t_epsilon:
                    solver_w_la = []
                    a = w_tri*w_test*dx
                    for k in range(self.options.n_layers):
                        ...
                        if consider_vert_adv:
                            ...
                            if consider_vertical_mass_flux:
                                 l += -self.dt/h_layer[k+1]*inner(m_dic['z_'+str(k+1)]*(w_dic['z_'+str(k+1)] - getattr(self, 'w_'+str(k)+str(k+1))) -
                                                                  m_dic['z_'+str(k)]*(w_dic['z_'+str(k)] - getattr(self, 'w_'+str(k)+str(k+1))), w_test)*dx
                        prob = LinearVariationalProblem(a, l, getattr(self, 'w_'+str(k)+str(k+1)))
                        solver_w_la.append(LinearVariationalSolver(prob))

                for k in range(self.options.n_layers):
                    solver_w_la[k].solve()

                if self.simulation_time <= t_epsilon:
                    h_mid = self.bathymetry_dg + split(self.fields.solution_2d)[1]
                    if self.options.use_wetting_and_drying and not self.options.thin_film:
                        h_mid += 2.*self.options.wd_mindep**2 / (2.*self.options.wd_mindep + abs(h_mid)) + 0.5*(abs(h_mid) - h_mid)
                    # update layer thickness and z-coordinate
                    h_layer = [h_mid*alpha[0]]
                    ...
                    for k in range(self.options.n_layers):
                        self.timestepper.F += self.dt/h_mid*inner(grad((getattr(self, 'q_'+str(k)) + getattr(self, 'q_'+str(k+1)))/2.*h_layer[k+1]), uta_test)*dx
                        if k == self.options.n_layers - 1:
                            self.timestepper.F += self.dt/h_mid*inner(getattr(self, 'q_'+str(0))*grad(z_dic['z_'+str(0)]) - 
