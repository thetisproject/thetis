    ...
    **Example**
    ...
    def create_fields(self):
        ...
        if self.options.use_limiter_for_tracers and self.options.polynomial_degree > 0:
            self.tracer_limiter = limiter_class(self.function_spaces.H, self.fields.v_elem_size_3d)
        else:
            self.tracer_limiter = None
        if (self.options.use_limiter_for_velocity
                and self.options.polynomial_degree > 0
                and self.options.element_family == 'dg-dg'):
            self.uv_limiter = limiter_class(self.function_spaces.U, self.fields.v_elem_size_3d)
        else:
