...
class SSPRK22ALE(TimeIntegrator):
    ...
    # below is for the nh solver with ssprk in NHWAVE TODO modify or delete
    def stage_two_prep_nh(self):
        """
        Preprocess 2nd stage: compute all forms on the old geometry
        """
        if self._nontrivial:
            # Update $Mu$ with non-hydrostatic pressure
            assemble(self.mu_form, self.mu)
            # Evaluate $k = \Delta t F(u)$
            with timed_region('pre2_asseble_f'):
                assemble(self.l, self.tendency)
            # $q = \frac{1}{2}q + \frac{1}{2}q_{old} + \frac{1}{2}k$
            with timed_region('pre2_incr_rhs'):
                self.mu.assign(self.mu + self.tendency)

    def prepare_stage_nh(self, i_stage, t, update_forcings=None):
        """
        Preprocess stage i_stage.
        ...
        if i_stage == 0:
            ...
            self.stage_two_prep_nh()

    def solve_stage_nh(self, i_stage):
        """Solves i-th stage"""
        if i_stage == 0:
            self.stage_one_solve()
        else:
    ...
    def solve_pg_nh(self, i_stage, l_pg=None):
        if l_pg is not None:
            # Update $Mu$ with non-hydrostatic pressure
            assemble(l_pg, self.tendency)
            if i_stage == 0:
                self.mu.assign(self.mu + self.tendency)
            else:
                self.mu.assign(0.5*self.mu + 0.5*self.mu_old + 0.5*self.tendency)
            ...
            if i_stage == 1:
                assemble(self.mu_form, self.mu)
                self.mu.assign(0.5*self.mu + 0.5*self.mu_old)
                # solve
                assemble(self.a, self.lin_solver.A)
                self.lin_solver.solve(self.solution, self.mu) 
