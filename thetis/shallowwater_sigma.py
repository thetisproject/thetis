...
class ExternalPressureGradientTerm(ShallowWaterMomentumTerm):
    ...
    def residual(self, uv, eta, uv_old, eta_old, fields, fields_old, bnd_conditions=None):
        ...
        if grad_eta_by_parts:
            ...
            if uv is not None:
                head_star = avg(head) + 0.5*sqrt(avg(total_h)/g_grav)*jump(uv, self.normal[0])
            else:
                head_star = avg(head)
            f += g_grav*head_star*jump(self.u_test, self.normal[0])*self.dS
            for bnd_marker in self.boundary_markers:
                ...
                if funcs is not None:
                    ...
                    un_jump = inner(uv - uv_ext, self.normal[0])
                    eta_rie = 0.5*(head + eta_ext) + sqrt(total_h/g_grav)*un_jump
                    f += g_grav*eta_rie*dot(self.u_test, self.normal[0])*ds_bnd
                if funcs is None or 'symm' in funcs:
                    # assume land boundary
                    # impermeability implies external un=0
                    un_jump = inner(uv, self.normal[0])
                    head_rie = head + sqrt(total_h/g_grav)*un_jump
                    f += g_grav*head_rie*dot(self.u_test, self.normal[0])*ds_bnd
        else:
            ...
            for bnd_marker in self.boundary_markers:
                ...
                if funcs is not None:
                    ...
                    un_jump = inner(uv - uv_ext, self.normal[0])
                    eta_rie = 0.5*(head + eta_ext) + sqrt(total_h/g_grav)*un_jump
                    f += g_grav*(eta_rie-head)*dot(self.u_test, self.normal[0])*ds_bnd
        return -f
...
class HUDivTerm(ShallowWaterContinuityTerm):
    ...
    def residual(self, uv, eta, uv_old, eta_old, fields, fields_old, bnd_conditions=None):
        ...
        if hu_by_parts:
            ...
            if self.eta_is_dg:
                h = avg(total_h)
                uv_rie = avg(uv) + sqrt(g_grav/h)*jump(eta, self.normal[0])
                hu_star = h*uv_rie
                f += inner(jump(self.eta_test, self.normal[0]), hu_star)*self.dS
            for bnd_marker in self.boundary_markers:
                funcs = bnd_conditions.get(bnd_marker)
                ds_bnd = ds(int(bnd_marker), degree=self.quad_degree)
                if funcs is not None:
                    ...
                    un_rie = 0.5*inner(uv + uv_ext, self.normal[0]) + sqrt(g_grav/h_av)*eta_jump
                    un_jump = inner(uv_old - uv_ext_old, self.normal[0])
            ...
            for bnd_marker in self.boundary_markers:
                ...
                if funcs is None or 'un' in funcs:
                    f += -total_h*dot(uv, self.normal[0])*self.eta_test*ds_bnd
        return -f
