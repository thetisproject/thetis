...
class VerticalGradSolver(object):
    ...
    def __init__(self, source, solution, solver_parameters=None):
        ...
        a = inner(test, tri)*dx
        p = self.source
        l = -inner(p, Dx(test, vert_ind))*dx
        l += avg(p)*jump(test, normal[vert_ind])*dS_h
        l += p*test*normal[vert_ind]*(ds_t + ds_b)
...
class ShearFrequencySolver(object):
    ...
    def __init__(self, uv, m2, mu, mv, mu_tmp, relaxation=1.0, minval=1e-12):
        ...
        self.var_solvers = []
        for i_comp in range(vert_ind):#range(2):
...
class PsiSourceTerm(TracerTerm):
    ...
    def residual(self, solution, solution_old, fields, fields_old, bnd_conditions=None):
        ...
        f += diff_flux*self.test*self.normal[vert_ind]*ds_bottom
        # surface condition
        elem_frac = Constant(0.5)
        ...
        f += diff_flux*self.test*self.normal[vert_ind]*ds_surf
