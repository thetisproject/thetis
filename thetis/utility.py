...
from .adr_model import ADR_Model
...
def get_species_key_order(filename):
    r"""
    Returns a list of species keys in the same order as they appear in the input file.
    ...
    with open(filename, 'r') as f_in:
        ...
        for line in f_in:
            ...
            line = line.split(':')[0]
            # Find the line where the species block begins
            if not in_species_block:
                in_species_block = line == 'species'
                continue
            # Determine current key (which may or may not turn out to be a species key).
            matches = re.findall(r'[^\s]+', line)
            if len(matches) != 1:
                continue
            current_key = matches[0]
            # Determine level of indentation
            current_indentation = line.index(current_key)
            # If not indented, the species block must have ended, and the
            # rest of the file can be skipped.
            if current_indentation == 0:
                break
            # If this is the first key within the species block, record the level of indentation
            if key_indentation is None:
                key_indentation = current_indentation
            # Find all other keys with the same level of indentation as the first one
            if current_indentation != key_indentation:
                continue
            # Append each key to the output list
            species_keys.append(current_key)
    return species_keys
...
def read_tracer_from_yml(filename, function_space, lambdify_modules=None,
    ...
    key_order = None
    if preserve_order:
        # Ensure that parse_tracers_from_dict adds tracers in the same order
        # as the corresponding species appear in the input file.
        key_order = get_species_key_order(filename)
        # Ensure that key_order contains the correct set of keys
        expected_keys = set(model_dict['species'].keys())
        assert len(key_order) == len(expected_keys)
...
def parse_tracers_from_dict(model_dict, function_space, lambdify_modules=None,
    ...
    :arg model_dict: Dictionary defining species in an ADR system.
        This must include the keys used to identify each species, their
        full names, their diffusivities and their reaction terms.
