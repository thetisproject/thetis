...
def get_facet_mask(function_space, mode='geometric', facet='bottom'):
    ...
    .. note::
        ...
        z axis).
    """
    section, iset, facets = function_space.boundary_masks[mode]
    ifacet = -2 if facet == 'bottom' else -1
    off = section.getOffset(facets.indices[ifacet])
    dof = section.getDof(facets.indices[ifacet])
    indices = iset.indices[off:off+dof]
    return indices
