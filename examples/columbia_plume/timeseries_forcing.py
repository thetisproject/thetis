...
def test():
    ...
    init_date = datetime.datetime(2015, 5, 1, tzinfo=sim_tz)
    print('sim. start: {:} {:}'.format(
        timezone.datetime_to_epoch(datetime.datetime(2015, 5, 1, 8, tzinfo=timezone.pytz.utc)),
    ...
    ncd = netCDF4.Dataset('forcings/stations/bvao3/bvao3.0.A.FLUX/201505.nc')
    t = ncd['time'][:]
    v = ncd['flux'][:]
