...
def test():
    ...
    print('{:} {:}'.format(timezone.datetime_to_epoch(datetime.datetime(1970, 1, 1, tzinfo=timezone.utc_tz)), timezone.datetime_to_epoch(init_date)))

    print('{:} {:}'.format(timezone.datetime_to_epoch(datetime.datetime(2016, 5, 1, 8, tzinfo=timezone.utc_tz)), timezone.datetime_to_epoch(datetime.datetime(2016, 5, 1, tzinfo=sim_tz))))
    ...
    print('{:} {:}'.format(timezone.datetime_to_epoch(init_date), init_date, river_flux_interp(0.)))

    ncd = netCDF4.Dataset('forcings/stations/bvao3/bvao3.0.A.FLUX/201605.nc')
    t = ncd['time'][:]
    v = ncd['flux'][:]
    print('{:} {:} {:}'.format(t[0], datetime.datetime.fromtimestamp(t[0], tz=timezone.utc_tz), v[0]))
