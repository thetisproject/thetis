# standing wave in a rectangular basin, solved by a solver in conservative form
# ===================
# Wei Pan 2018-01-08
from thetis import *

lx = 20.0
ly = 2
nx = 20
ny = 1
mesh = RectangleMesh(nx, ny, lx, ly)
x, y = SpatialCoordinate(mesh)
depth = 100
elev_amp = 0.1
n_layers = 5
outputdir = 'outputs_standing_wave_cf_p0dg-vert'
print_output('Exporting to ' + outputdir)

# bathymetry
P1_2d = FunctionSpace(mesh, 'CG', 1)
bathymetry_2d = Function(P1_2d, name='Bathymetry')
bathymetry_2d.assign(depth)

# set time step, export interval and run duration
dt = 0.005
t_export = 0.1
t_end = 20.

# --- create solver ---
solver_obj = nhsolver_cf.FlowSolver(mesh, bathymetry_2d, n_layers)
options = solver_obj.options
options.element_family = 'dg-dg'
options.polynomial_degree = 1
options.timestepper_type = 'SSPRK22'
options.timestepper_options.use_automatic_timestep = False
# free surface elevation
options.update_free_surface = True
options.solve_separate_elevation_gradient = True
options.solve_conservative_momentum = True
options.use_vert_dg0 = True
# tracer
options.solve_salinity = False
options.solve_temperature = False
options.use_implicit_vertical_diffusion = False
# limiter
options.use_limiter_for_velocity = False
options.use_limiter_for_tracers = False
options.use_limiter_for_elevation = False
# time
options.timestep = dt
options.simulation_export_time = t_export
options.simulation_end_time = t_end
# output
options.output_directory = outputdir
options.fields_to_export = ['elev_2d', 'elev_3d', 'mom_3d']
# flux
options.use_hllc_flux = False
# wetting and drying
options.use_wetting_and_drying = False
options.wetting_and_drying_threshold = 1e-6
options.depth_wd_interface = 1e-3

# need to call creator to create the function spaces
solver_obj.create_equations()

# set initial elevation
elev_init = Function(solver_obj.function_spaces.H_2d)
elev_init.interpolate(elev_amp*cos(2*pi*x/lx))

solver_obj.assign_initial_conditions(elev_2d=elev_init)

solver_obj.iterate()

# error show
anal_elev = Function(solver_obj.function_spaces.H_2d).interpolate(elev_amp*cos(2*pi*x/lx)*cos(sqrt(9.81*2*pi/lx)*t_end))
L2_elev = errornorm(anal_elev, solver_obj.fields.elev_2d)/sqrt(lx*ly)

print (L2_elev)

