...
lat2metre = lambda lat: earth_radius * deg2rad(lat)
metre2lat = lambda m: rad2deg(m / earth_radius)
...
def okada(parameters, x, y):
    ...
    Yoshimitsu Okada, "Surface deformation due to shear
      ...
      pp.1135--1154, (1985).
    """
    assert isinstance(parameters, OkadaParameters)
    P = parameters
    half_length = 0.5 * P.length
    poisson = 0.25  # Poisson ratio

    # Get centres
    x_bot = P.lon - 0.5 * metre2lat(-P.width * deg2cos(P.dip) * deg2cos(P.strike) / deg2cos(P.lat))
    y_bot = P.lat - 0.5 * metre2lat(P.width * deg2cos(P.dip) * deg2sin(P.strike))
    z_bot = P.depth + 0.5 * P.width * deg2sin(P.dip)

    # Convert from degrees to metres
    xx = lat2metre(deg2cos(y) * (x - x_bot))
    yy = lat2metre(y - y_bot)
